        ...
        if ($comment['parent_id'] == $parent_id) {
            ...
            <div class="comment">
                <div>
                    ...
                    <div class="replies">
                    ' . show_comments($comment, $comment['id']) . '
                    </div>
            </div>
            ';
        }
...
if (isset($_GET['page_id'])) {
    if (isset($_POST['name'], $_POST['content'])) {
        $stmt = $pdo->prepare('INSERT INTO comment (page_id, parent_id, name, content, submit_date) VALUES (?,?,?,?,NOW())');
        $stmt->execute([$_GET['page_id'], $_POST['parent_id'], $_POST['name'], $_POST['content']]);
        exit('Your comment has been submitted!');
    }

    $stmt = $pdo->prepare('SELECT * FROM comment WHERE page_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['page_id']]);
    $comments = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $stmt = $pdo->prepare('SELECT COUNT(*) AS total_comments FROM comment WHERE page_id = ?');
    $stmt->execute([$_GET['page_id']]);
